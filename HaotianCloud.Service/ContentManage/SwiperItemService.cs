...
using HaotianCloud.Domain.ContentManage;
using Chloe;

namespace HaotianCloud.Service.ContentManage
{
    /// <summary>
    /// 创 建：超级管理员
    ...
    /// 描 述：首页轮播图广告服务类
    /// </summary>
    public class SwiperItemService : DataFilterService<SwiperItemEntity>, IDenpendency
    {
        public SwiperItemService(IDbContext context) : base(context)
        {

        }
        private string cacheKey = "HaotianCloud_cms_SwiperItemdata_";
        
        #region 获取数据
        public async Task<List<SwiperItemEntity>> GetList(string keyword = "")
        {
        ...
        public async Task<List<SwiperItemEntity>> GetLookList(SoulPage<SwiperItemEntity> pagination, string keyword = "", string CategoryId="")
        {
            //反格式化显示只能用"等于"，其他不支持
            ...
            .Select((a, b) => new SwiperItemEntity
            {
                F_Id = a.F_Id,
            ...
            //权限过滤
            query = GetDataPrivilege<SwiperItemEntity>("u", "",query);           
        ...
        /// <returns></returns>
        public async Task<SwiperItemEntity> GetForm(string keyValue)
        {
            var query = repository.IQueryable(a => a.F_EnabledMark == true)
                .LeftJoin<ArticleCategoryEntity>((a, b) => a.F_CategoryId == b.F_Id && b.F_EnabledMark == true)
                .Select((a, b) => new SwiperItemEntity
        ...
        #region 提交数据
        public async Task SubmitForm(SwiperItemEntity entity, string keyValue)
        {
